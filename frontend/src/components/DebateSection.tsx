...
import { debateCost } from "@/lib/tokenCosts";
...
export default function DebateSection({
  ...
  const [selectedMetrics, setSelectedMetrics] = useState<string[]>(metricsToDebate);
  ...
  const isRedebate = !!debateData;
  const activeMetrics = isRedebate ? selectedMetrics : metricsToDebate;

  const canStart = selectedModels.length >= 2 && activeMetrics.length >= 1 && !debating;
  const estimatedCost = debateCost(selectedModels, activeMetrics.length, rounds);
  ...
  return (
    <div className="glass-card rounded-xl p-4 md:p-6 flex flex-col gap-5 animate-fadeIn">
      ...
      {/* Metric selection — only visible after first debate */}
      {isRedebate && !debating && (
        <div className="flex flex-col gap-2 animate-fadeIn">
          <span className="text-[10px] uppercase tracking-wider text-muted font-mono text-center">
            Re-debate metrics
          </span>
          <div className="flex flex-wrap gap-2 justify-center">
            {metricsToDebate.map((metric) => (
              <label
                key={metric}
                className={`flex items-center gap-2 px-3 py-2 md:py-1.5 rounded-lg border text-xs font-mono cursor-pointer transition-colors ${
        ...
        <div className="flex flex-col items-center gap-1.5">
          <button
            ...
            onClick={() => {
              ...
              if (tokenBalance !== undefined && tokenBalance < estimatedCost) {
                onFeatureGate?.(`Insufficient tokens. This debate costs ~${estimatedCost} tokens but you have ${tokenBalance}.`);
                return;
              }
              onDebate(selectedModels, activeMetrics, rounds);
            }}
          ...
          {isLoggedIn && canStart && !debating && (
            <span className="text-[10px] font-mono text-sky-300/70">~{estimatedCost} tokens</span>
          )}
        </div>
      ) : (
      ...
      {isRedebate && activeMetrics.length === 0 && !debating && (
        <p className="text-[10px] text-red-400/70 font-mono text-center">
          Select at least 1 metric to re-debate
        </p>
      )}
      ...
      {debateError && !debating && (
        <div className="flex items-start gap-3 p-3 rounded-lg bg-red-400/10 border border-red-400/20 animate-fadeIn">
          ...
          <button
            onClick={() => onDebate(selectedModels, activeMetrics, rounds)}
