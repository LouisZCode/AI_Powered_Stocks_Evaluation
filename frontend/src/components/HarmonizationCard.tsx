...
function actionBadge(action: HarmonizationLogEntry["action"]) {
  switch (action) {
    ...
    case "harmonized":
      return { label: "Agreed", cls: "text-emerald-400 bg-emerald-400/15 border-emerald-400/25", glow: "rgba(52, 211, 153, 0.35)" };
    case "debate":
      return { label: "Unresolved", cls: "text-amber-300 bg-amber-300/15 border-amber-300/25", glow: "rgba(252, 211, 77, 0.35)" };
    case "skipped":
...
export default function HarmonizationCard({ data, reveal = true }: Props) {
  ...
  const agreedCount = summary.already_aligned + summary.harmonized;
    ...
    <div className="glass-card rounded-xl p-4 md:p-6 flex flex-col gap-5 animate-fadeIn">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 text-xs font-mono">
          <span className="text-emerald-400">{agreedCount} agreed</span>
          {summary.needs_debate > 0 && (
            <>
              <span className="text-muted">·</span>
              <span className="text-amber-300">{summary.needs_debate} unresolved</span>
            </>
          )}
        </div>
      </div>
      ...
      {reveal && data.metrics_to_debate.length > 0 && (
        <div className="border-t border-white/[0.08] pt-3 animate-fadeIn">
          <p className="text-xs text-muted">
            <span className="text-amber-300 font-medium">{data.metrics_to_debate.length} metric{data.metrics_to_debate.length > 1 ? "s" : ""}</span>{" "}
